...
const ProgressTracker: React.FC = () => {
  ...
  const { progress, updateProgress, calculateICanProgress, unitProgressCalculations, overallCourseProgress } = useStudentProgress();
  ...
  return <div className="flex min-h-[calc(100vh-73px)] flex-col md:flex-row">
    ...
    <main className="min-w-0 flex-1"><UnitView unit={selectedUnit} studentProgress={progress} onStatusChange={updateProgress} calculateICanProgress={calculateICanProgress} onVocabClick={setSelectedVocabTerm} unitProgress={unitProgressCalculations[selectedUnit.id] || 0} /></main>
  </div>;
};
